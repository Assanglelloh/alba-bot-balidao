(function () {
  ...
  style.textContent = [
    '#alba-wrapper * { box-sizing: border-box; font-family: -apple-system, BlinkMacSystemFont, "Segoe UI", sans-serif; }',
    ...
    '#alba-window { position: fixed; bottom: 90px; right: 24px; width: 360px; max-height: 540px; border-radius: 16px; background: #fff; box-shadow: 0 8px 32px rgba(0,0,0,0.18); display: flex; flex-direction: column; z-index: 99999; overflow: hidden; transform-origin: bottom right; transition: transform .25s cubic-bezier(.4,0,.2,1), opacity .25s; }',
    ...
    '#alba-header .alba-close { background: none; border: none; color: #fff; cursor: pointer; font-size: 20px; padding: 0; line-height: 1; opacity: .8; }',
    '#alba-header .alba-close:hover { opacity: 1; }',
    '#alba-messages { flex: 1; overflow-y: auto; padding: 16px; display: flex; flex-direction: column; gap: 10px; background: #f9f9f9; }',
    '#alba-messages::-webkit-scrollbar { width: 4px; }',
    '#alba-messages::-webkit-scrollbar-thumb { background: #e0e0e0; border-radius: 4px; }',
    ...
    '#alba-input { flex: 1; border: 1px solid #e0e0e0; border-radius: 20px; padding: 9px 14px; font-size: 14px; outline: none; resize: none; height: 40px; overflow: hidden; transition: border-color .2s; }',
    ...
    '@media (max-width: 420px) { #alba-window { width: calc(100vw - 16px); right: 8px; bottom: 80px; } #alba-bubble { right: 12px; bottom: 12px; } }',
  ].join('\n');
  ...
  wrapper.innerHTML = [
    '<button id="alba-bubble" aria-label="Chat avec Alba">',
    '  <span style="font-size:24px">💬</span>',
    '  <span class="alba-notif"></span>',
    '</button>',
    '<div id="alba-window" class="hidden" role="dialog" aria-label="Alba — Assistante Balidao">',
    '  <div id="alba-header">',
    '    <div class="alba-avatar">' + BOT_AVATAR + '</div>',
    '    <div class="alba-info">',
    '      <div class="alba-name">' + BOT_NAME + ' — Balidao</div>',
    '      <div class="alba-status">🟢 En ligne • Répond en quelques secondes</div>',
    '    </div>',
    '    <button class="alba-close" aria-label="Fermer">✕</button>',
    '  </div>',
    '  <div id="alba-messages"></div>',
    '  <div id="alba-footer">',
    '    <input id="alba-input" type="text" placeholder="Posez votre question..." maxlength="500" autocomplete="off" />',
    '    <button id="alba-send" aria-label="Envoyer">',
  ...
  var send   = document.getElementById('alba-send');
  ...
  async function sendMessage() {
    ...
    try {
      ...
      typing.remove();
      addMsg(data.reply || "Désolée, je n'ai pas pu répondre. Réessayez !", 'bot');
    } catch (e) {
      typing.remove();
      addMsg("Je rencontre un problème de connexion. Contactez-nous sur WhatsApp : https://wa.me/22899231818 😊", 'bot');
    }
  }
  ...
  // Ouvrir automatiquement après 3s pour accueillir le visiteur
  setTimeout(function () {
    if (!open) {
      notif.style.display = 'none';
      toggleWindow();
